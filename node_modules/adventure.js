...
var Adventure = function(config){
	...
	console.log('Adventure Module connected: '+config.databaseUrl+ ':' + config.port);
	...
	db.query("START n = node(0) MATCH (n) <-[:COLLECTIONS_REFERENCE]- () <-[:ADVENTURES_REFERENCE]- (adv_ref) RETURN adv_ref", function(errors, nodes) {
		if (errors) {
			...
			// console.log(ADVENTURES_REFERENCE);
		}
	});
	...
	return {
		create: function(req, res, next){
			...
			var adventureData = {
				...
				startDate: newAdventure.startDate,
				endDate: newAdventure.endDate
			...
			Step(
				function getInviteJoins(){
					if(newAdventure.requestJoin.length) {
						...
						for (var i=0, j=newAdventure.requestJoin.length; i<j; i++) {
							db.getNodeById( newAdventure.requestJoin[i].id, group() );
						}
					} else {
						return [];
					}
				...
				function getInviteFollows(){
					if(newAdventure.requestFollow.length) {
						...
						for (var i=0, j=newAdventure.requestFollow.length; i<j; i++) {
							db.getNodeById( newAdventure.requestFollow[i].id, group() );
						}
					} else {
						return [];
					}
				...
				function relateAdventure(err){
					console.log(adventureNode, adventureOwner);
				...
				function sendInviteJoins(err){
					var invite = adventureOwner._data.data.first + ' has invited you to join ' +
						adventureData.title;
				...
				function sendInviteFollows(){
					var invite = adventureOwner._data.data.first + ' has invited you to follow ' +
						adventureData.title;
					if(userInviteJoins.length) {
						var group = this.group();
						userInviteJoins.forEach(function(user){
