...
module.exports = function(configOptions){
	...
	var thumbNailProcessComplete = function(err, results, callback) {
		if(scopeSocket) {
			if(!err) {
				console.log('image results done, sending to socket', results);
		...
		Step(
			function getImages(){
				...
				if(typeof images == 'string') {
					console.log('user create function was called with a token', images, remoteUserPath);
					// gather images array
			...
			function uploadImages(err, results){
				console.log('upload images!', arguments);
				var group = this.group();
				console.log("here is all the shit...", imageArray);
	...
	return {
		...
		acceptAsyncUpload: function(req, res, next) {
			...
			form.parse(req, function(err, fields, files){
				console.log('form data done, now parsing...');

				if(!err) {
					var token = files.photo.path.split('/').pop();
					console.log(token);
					scopeSocket.emit('uploadComplete', { action: 'uploading', value: token });
					scopeSocket.emit('processingThumbs', { action: 'processing', value: 0 });
					console.log("now fire the image thumbnails steps please");
			...
			form.on('progress', function(bytesReceived, bytesExpected){
				...
				if(scopeSocket) {
					...
					if(percent > 0.4) {
						scopeSocket.emit('uploadConnect', 'BAHA UPLOAD RECONNECT BUG');
					}
				}
			});
		}
