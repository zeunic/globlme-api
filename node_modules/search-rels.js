...
var SearchRels = function(relType,database){
	...
	var prepQuery = function(type, direction, startID){

		var REL_AND_DIRECTION = (direction == 'in') ? ('-[rel?:' + REL_TYPE + ']->') : ('<-[rel?:' + REL_TYPE + ']-');
		var nodeDomainID;

		switch(type) {
			case "tags":
				nodeDomainID = TAG_REFERENCE.id;
				break;
			case "users":
				nodeDomainID = USER_REFERENCE.id;
			break;
			default:
				nodeDomainID = USER_REFERENCE.id;
		}
		...
		tags: function(type, direction, startID, callback){
			Step(
				function getQueryString(){
					return prepQuery(type, direction, startID);
				},
